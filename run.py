...
import json
import random
from evaluation_lama import evaluate_lama #zzh
from evaluation_perp import evaluate_perp #zzh
import numpy as np
import torch
...
if __name__ == '__main__':
    ...
    if 't5' or 'T5' in args.model_name_or_path: 
        model_type='T5'
    ...
    if args.check_validation == 'lama':
        evaluate_lama(args, Model) #zzh 更改config文件，区分两种evaluation
    elif args.check_validation == 'perp':
        evaluate_perp(args, Model)
    else:
        set_seed(40)
        ...
        trainer.fit(model) #zzh 为什么没有制定ckpt文件的位置？
